/*
  ...
  This version modified by P Harrison 2021
  Uses constexpr functions in place of some macros so that cppcheck is happy
 */
...
#define __digitalPinToBit(P)      (((P) >= 0 && (P) <= 7) ? (P) : (((P) >= 8 && (P) <= 13) ? (P)-8 : (P)-14))
   */
constexpr volatile uint8_t *__digitalPinToPortReg(uint8_t P) {
  return (((P) <= 7) ? &PORTD : (((P) >= 8 && (P) <= 13) ? &PORTB : &PORTC));
};
constexpr volatile uint8_t *__digitalPinToDDRReg(uint8_t P) {
  return (((P) <= 7) ? &DDRD : (((P) >= 8 && (P) <= 13) ? &DDRB : &DDRC));
};
constexpr volatile uint8_t *__digitalPinToPINReg(uint8_t P) {
  return (((P) <= 7) ? &PIND : (((P) >= 8 && (P) <= 13) ? &PINB : &PINC));
};
constexpr uint8_t __digitalPinToBit(uint8_t P) {
  return (((P) <= 7) ? (P) : (((P) >= 8 && (P) <= 13) ? (P)-8 : (P)-14));
};
#endif
